...
p=ggplot(data=d1_s, aes(x=Term, y=ListHits, width=0.8, fill=Regulation,space=0)) +
  ...
  theme(plot.margin=unit(c(2,4,2,2), "lines"))+
  theme(panel.grid =element_blank()) 

ggsave(paste0(opt$outpath, "/GO(Biological_process).top.", "pdf"), height=10, width=10, plot=p)
ggsave(paste0(opt$outpath, "/GO(Biological_process).top.", "png"), type="cairo-png", height=10, width=10, plot=p)
...
p2=ggplot(data=d2_s, aes(x=Term, y=ListHits, width=0.8, fill=Regulation,space=0)) +
  ...
  theme(plot.margin=unit(c(2,4,2,2), "lines"))+
  theme(panel.grid =element_blank()) 
ggsave(paste0(opt$outpath, "/GO(Cellular_component).top.", "pdf"), height=10, width=10, plot=p2)
ggsave(paste0(opt$outpath, "/GO(Cellular_component).top.", "png"), type="cairo-png", height=10, width=10, plot=p2)
...
ggsave(paste0(opt$outpath, "/GO(Molecular_function).top.", "pdf"), height=10, width=10, plot=p3)
ggsave(paste0(opt$outpath, "/GO(Molecular_function).top.", "png"), type="cairo-png", height=10, width=10, plot=p3)
print(paste0(opt$outpath, "/GO(Molecular_function).top.", "png(pdf) is OK"));


#############################画总图##############################
figure <- plot_grid(p,p2,p3, labels = c("BP", "CC", "MF"), ncol = 1, nrow = 3, align = "v", axis = "tlbr")

ggsave(paste0(opt$outpath, "/GO(MFBPCC).top.", "pdf"), height=10, width=10, plot=figure)
ggsave(paste0(opt$outpath, "/GO(MFBPCC).top.", "png"), type="cairo-png", height=10, width=10, plot=figure)
print(paste0(opt$outpath, "/GO(MFBPCC).top.", "png(pdf) is OK"));
