...
top10 <- function(i) { return(i[order(head(i, Number)["pValue"]),]) }
...
dp <- rbind(top10(d[d["Regulation"]=="Up", ]),
	top10(d[d["Regulation"]=="Down", ]),
	top10(d[d["Regulation"]=="", ]))
write.table(dp[,c(1,2,3,4,8,10,11)], paste0(opt$outpath, "/kegg.top.", opt$mark, ".xls"), sep="\t", quote=FALSE,
	col.names=TRUE, row.names=FALSE)

d1<-dp[which(dp[,3]=="Up"),]
#d1_s<-d1[order(d1[,4],decreasing=F),]
d2<-dp[which(dp[,3]=="Down"),]
...
p=ggplot(data=d_l, aes(x=Term, y=Enrichment_score, width=0.6, fill=pValue,space=0.6,cex.main=3,cex.lab=2)) +
  geom_bar(stat="identity",position=position_dodge(0.7),width=0.5) +
  labs(x="", y='Enrichment_score', fill='pValue', title = paste0(groupname,": ","Top ",opt$Number, " Up & Down KEGG Term")) +
  ...
  theme(panel.grid =element_blank()) + facet_wrap(~Regulation,scales="free") 
