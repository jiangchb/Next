...
d3 <- d
top20 <- function(i) { return(i[order(head(i, 20)["ListHits"],decreasing=F),]) }
#先筛选和排序

top10 <- function(i) { return(i[order(head(i, Number)["ListHits"],decreasing=F),]) }
#除了p值外，其他都是升序
#stopifnot(nrow(d)>0)
if(nrow(d)==0){
...
#分图##################输出参考表格################
dp <- rbind(top20(d[d["Category"]=="biological_process", ]),
            top20(d[d["Category"]=="cellular_component", ]),
            top20(d[d["Category"]=="molecular_function", ]))
write.table(dp[,c(1,2,3,4,5,6,7,8,9)], paste0(opt$outpath, "/GO(BPCCMF).top60.", opt$mark, ".xls"), sep="\t", quote=FALSE,
...
d1_l<-d1[order(d1[,4],decreasing=T),]
...
p=ggplot(data=d1_s, aes(x=Term, y=ListHits, width=0.5,space=0),col="blue") + #fill=Regulation
  coord_flip()+
  geom_bar(stat="identity",fill="#336699",position=position_dodge(1),width=0.9) +
  scale_x_discrete(breaks=d1_s$Term, labels=d1_s$Term)+ 
  guides(fill=guide_legend(reverse=TRUE))+
  scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label = ListHits), size = 4,vjust=-0.5,hjust=-0.5, force = 1,color = "black") +
  labs(x="", y='Number of gene', title = paste0(groupname,": ","Top ",opt$Number, "(Biological Process) go Term"))+  
  theme_bw() +
...
ggsave(paste0(opt$outpath, "/GO(Biological_process).top.", "pdf"), height=10, width=16, plot=p)
ggsave(paste0(opt$outpath, "/GO(Biological_process).top.", "png"), type="cairo-png", height=10, width=16, plot=p)
...
d2_l<-d2[order(d2[,4],decreasing=T),]
...
p2=ggplot(data=d2_s, aes(x=Term, y=ListHits, width=0.5,space=0),col="blue") +#, fill=Regulation
  coord_flip()+
  geom_bar(stat="identity",fill="#336699",position=position_dodge(1),width=0.9) +
  ...
  scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label = ListHits), size = 4, vjust=-0.5,hjust=-0.5, force = 1,color = "black") +
  labs(x="", y='Number of gene', title = paste0(groupname,": ","Top ",opt$Number, "(Cellular component) go Term"))+  
  theme_bw() +
  theme(axis.text.y=element_text(size=15,color="black")) + 
  theme(axis.text.x=element_text(hjust=1, size=14))+
  theme(legend.position=c(-0.10,0.8), legend.key.width=unit(1, "lines"))+
  ...
  theme(plot.margin=unit(c(2,4,2,18), "lines"))+
  theme(panel.grid =element_blank()) 
ggsave(paste0(opt$outpath, "/GO(Cellular_component).top.", "pdf"), height=10, width=16, plot=p2)
ggsave(paste0(opt$outpath, "/GO(Cellular_component).top.", "png"), type="cairo-png", height=10, width=16, plot=p2)
print(paste0(opt$outpath, "/GO(Cellular_component).top.", "png(pdf) is OK"));

######################################画MF################################
d3<-dp[which(dp[,3]=="molecular_function"),]
d3_l<-d3[order(d3[,4],decreasing=T),]
...
p3=ggplot(data=d3_s, aes(x=Term, y=ListHits, width=0.5,space=0),col="blue") + #, fill=Regulation
  coord_flip()+
  geom_bar(stat="identity",fill="#336699",position=position_dodge(1),width=0.9) +
  scale_x_discrete(breaks=d3_s$Term, labels=d3_s$Term)+ 
  guides(fill=guide_legend(reverse=TRUE))+
  scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label = ListHits), size = 4, vjust=-0.5,hjust=-0.5,color = "black") +#
  labs(x="", y='Number of gene', title = paste0(groupname,": ","Top ",opt$Number, "(Molecular function) go Term"))+  
  theme_bw() +
  theme(axis.text.y=element_text(size=15,color="black")) + 
  theme(axis.text.x=element_text(hjust=1, size=14))+
  theme(legend.position=c(-0.10,0.8), legend.key.width=unit(1, "lines"))+
...
ggsave(paste0(opt$outpath, "/GO(Molecular_function).top.", "pdf"), height=10, width=16, plot=p3)
ggsave(paste0(opt$outpath, "/GO(Molecular_function).top.", "png"), type="cairo-png", height=10, width=16, plot=p3)
