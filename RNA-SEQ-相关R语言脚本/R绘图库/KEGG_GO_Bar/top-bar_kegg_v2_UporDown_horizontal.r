...
top10 <- function(i) { return(i[order(head(i, Number)["Enrichment_score"]),]) }

d <- read.delim(opt$input, sep="\t", header=T, quote="")
d <- d[which(d[,"ListHits"]>0),]
d <- d[order(d[,"Enrichment_score"]),]
...
write.table(d_l[,c(1,2,3,4,8,10,11)], paste0(opt$outpath, "/kegg.top.", opt$mark, ".xls"), sep="\t", quote=FALSE,
...
p=ggplot(data=d_l, aes(x=Term, y=Enrichment_score, width=0.6, fill=pValue,space=0.6,cex.main=3,cex.lab=2)) +
  ...
  labs(x="", y='Enrichment_score', fill='pValue', title = paste0(groupname,": ","Top ",opt$Number," KEGG Term")) +
