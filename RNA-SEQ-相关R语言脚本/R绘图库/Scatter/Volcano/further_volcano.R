...
if ( !is.null(opt$genelist) ) {
    ...
    if (length(rownames(labels)) <= 15) {
        ...
        scale_colour_manual(values = c("Up"=c(oe_col_qua(2)), "Down"=c(oe_col_qua(3)),"Filtered"=c(oe_col_qua(8)))) + 
        ...
        theme(plot.title = element_text(hjust = 0.5)) + 
        guides(shape=guide_legend(override.aes=list(size=4))) +
        scale_colour_manual(values = c("Up"=c(oe_col_qua(2)), "Down"=c(oe_col_qua(3)),"Filtered"=c(oe_col_qua(8)),"Labels"=c(oe_col_qua(5)))) + 
        theme(panel.grid = element_blank()) 
    }
}else {
    g = ggplot(data=DEG, aes(x=DEG$log2FoldChange, y=-log10(DEG$pValue), color=change)) + 
