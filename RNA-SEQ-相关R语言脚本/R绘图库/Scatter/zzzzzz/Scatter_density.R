...
p <- ggplot(Three, aes(x=logFC_A, y=logFC_B) )  + geom_bin2d(bins = 140) +
  scale_fill_continuous(type = "viridis") + theme_bw() +   geom_vline(xintercept = c(0, 0), lty = 2) +
  geom_hline(yintercept = c(0, 0), lty = 2) 
p

Four <- subset(two,select=c("logFC_A","logFC_B"))

p <- ggplot(Four, aes(x=logFC_A, y=logFC_B) )  + geom_bin2d(bins = 140) +
  scale_fill_continuous(type = "viridis") + theme_bw() +   geom_vline(xintercept = c(0, 0), lty = 2) +
  geom_hline(yintercept = c(0, 0), lty = 2) + stat_cor(data=dat, method = "pearson") 
p

ggsave('gene_two.png', p1, width = 7, height = 7)
...
p1 <- p + labs(x = paste0('log2FC ',"(",groupname,")"), y = paste0('log2FC ',"(",groupname2,')'),subtitle = as.expression(eq))
p1

ggsave(paste0("./",'DensityScatter-FC2-q-value-0.05.pdf'), p1, width = 7, height = 7)
ggsave(paste0("./",'DensityScatter-FC2-q-value-0.05.png'), p1, width = 7, height = 7)
