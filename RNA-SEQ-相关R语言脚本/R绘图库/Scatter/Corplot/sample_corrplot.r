#当样本量过多时，建议更改tl.cex,去掉addCol，去掉相关性系数显示
...
min<-min(matrix)
wid<-10+2*log2(length(colnames(data)))
hig<-10+2*log2(length(colnames(data)))
...
png(paste(opt$output,".png",sep=""),width = 5000, height = 5000, res = 300)
corrplot(matrix,
   tl.cex=(0.8+0.0001*log2(length(colnames(data)))),
   #title="Correlation Coefficient Between Samples",
   mar=c(0,0,0,0),
   method="circle",
   is.corr=FALSE,
