...
GetComplexHeatmap <- function (fpkm_matrix,term_name) {
  p=Heatmap(fpkm_matrix,show_column_names = T,show_row_names = F,name = "Color key",
            #clustering_distance_columns = "pearson",
            #clustering_distance_rows = "euclidean",
            cluster_rows = opt$rowcluster,
            cluster_columns = opt$colcluster,
            column_title_rot = 90,
            row_title_rot = 90,
            ...
            #heatmap_legend_param=list(title= "Color Key", legend_direction="horizontal")
            )
  return (p)
}
...
#####################出图######################################
pdf(paste0(output_dir,"/",groupname,".",opt$rowcluster,"-output.pdf"),width=9,height=18)
a=get(paste("pic",1,sep = ""))
for (i in 2:length(term_list)) {
  a=a %v% get(paste("pic",i,sep = "")) #本质上就是add_Heatmap
}
draw(a,main_heatmap = 1,
     #ht_gap=unit(c(3, 1), "mm"),
    gap=unit(1,"mm"),
     column_title = "Heatmaps of Gene Expression for Multiple GO Term", column_title_gp = gpar(fontsize = 16, fontface="bold"))
dev.off()
...
tmp_df=rbind(gene_id=row.names(b), b)
row.names(tmp_df)=NULL

for (i in 2:length(term_list)) {
  m2=get(paste("mat",i,sep = ""))
  m3=rbind(gene_id=row.names(m2), m2)
  row.names(m3)=NULL
