...
option_list = list(
	...
	make_option(c("-cut", "--cutrow"), type = "character", default = "1",
...
#palette <- colorRampPalette(c("red","green"))(n=256)
...
if (length(samples) < 2) {
    ...
    color = palette, show_rownames = show_rownames, fontsize=fontsize, border=border,
    cluster_rows = opt$rowcluster , cluster_cols = opt$colcluster , angle_col = angle, cutree_rows = opt$cutrow)
}
