...
####处理表达量文件
  ...
  write.table(tmp,file=paste0(output_dir,"/",term_list[i],".",opt$rowcluster,".split_result.xls"),row.names=F,quote = FALSE,sep='\t')
  tmp=scale(t(tmp[,-1]))
  ...
  assign(paste("pic", i, sep = ""), GetComplexHeatmap(tmp,
                                                        term_list[i]))
  pdf(paste0(output_dir,"/",term_list[i],".",opt$rowcluster, ".pdf"), height=10, width=18)
  draw(GetComplexHeatmap(tmp,
                    term_list[i]))
  dev.off()
