...
topn <- function(i) { return(i[order(head(i, Number)["Enrichment_score"],decreasing=T),]) }
...
ylabs="GeneRatio"
size.lab <- "GeneCounts"
p=ggplot(d_l,aes(x=Term,y=GeneRatio))+
  geom_point(aes(color=pValue,size=ListHits))+
  scale_colour_gradientn(colours=plasma(6)) + #need self-define
  scale_size_area(max_size = 10) + #adjust the size of the bubble
  theme(legend.justification=c(0,0),legend.position=c(1,1)) +
  coord_flip()+
  labs(x="", y=ylabs, title = paste0("Top ",Number," ",opt$mark, " KEGG enrichment"),size=size.lab,font.axis=2,font.lab=2,cex.axis=2) + 
...
write.table(d_l[,c(1,2,3,4,8,9,10,11,12)], paste0(opt$outpath, "/KEGG.gene_ratio_order.", mark, ".xls"), sep="\t", quote=FALSE,
	col.names=TRUE, row.names=FALSE)
ggsave(paste0(opt$outpath, "/KEGG.top.", mark, ".pdf"), height=10, width=7, plot=p)
ggsave(paste0(opt$outpath, "/KEGG.top.", mark, ".png"), type="cairo-png", height=10, width=7, plot=p)
print(paste0(opt$outpath, "/KEGG.top.", mark, ".png(pdf) is OK"));
