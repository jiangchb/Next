...
option_list = list(
	make_option(c("-bp", "--bpGOterm"), type="character", default=NULL, help="input file", metavar="character"),
	make_option(c("-mf", "--mfGOterm"), type="character", default=NULL, help="input file", metavar="character"),
	make_option(c("-cc", "--ccGOterm"), type="character", default=NULL, help="input file", metavar="character"),
...
endnode<-read.csv(opt$level2, header=F)
endnode<-as.matrix(endnode)

d.BP<-as.matrix(read.delim(opt$bpGOterm,header=T,sep="\t"))
d.MF<-as.matrix(read.delim(opt$mfGOterm,header=T,sep="\t"))
d.CC<-as.matrix(read.delim(opt$ccGOterm,header=T,sep="\t"))
