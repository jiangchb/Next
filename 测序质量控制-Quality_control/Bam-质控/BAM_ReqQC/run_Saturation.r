...
library(ggplot2)
library(reshape2)
outPDFfile <- paste(opt$outpath, "/", opt$sample, '.saturation.pdf', sep = '')
outPNGfile <- paste(opt$outpath, "/", opt$sample, '.saturation.png', sep = '')
...
p <- ggplot(dat, aes(x = x * 5, y = y, group = rpkm)) +
    ...
    ylim(0, 1) +
    xlab("Percent of Mapped reads (%)") +
    ylab("Fraction of Genes within 15% of Final Values") +
    labs(title = paste0("Saturation of Curve (", opt$sample, ")")) +
    scale_colour_discrete(guide = guide_legend(reverse = TRUE),
    name = "RPKM intervals",
    breaks = c("rate1", "rate2", "rate3", "rate4", "rate5"),
    labels = c(coln1, coln2, coln3, coln4, coln5))
