...
write.table(allRes,file=paste0(outputpath,"/","MF.",mark,".topGO_RESULT.xls"),col.names=T,row.names=F,quote = FALSE,sep='\t')

anno_gene <-max(allRes[,4])
if (anno_gene>2){
	png(paste0(outputpath, "/topGO_MF_",mark, opt$mark, ".png"), width=3000, height=3000, res=300)
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()

	pdf(paste0(outputpath, "/topGO_MF_",mark, opt$mark, ".pdf"))
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()
	print(paste0(outputpath, "/topGO_MF_",mark, opt$mark, ".pdf(png) is OK"));
}
...
#筛选并且出表格
allRes <- GenTable(GOdata,classicFisher = resultFisher, orderBy = "classicFisher", topNodes = 20)
write.table(allRes,file=paste0(outputpath,"/","BP.",mark,".topGO_RESULT.xls"),row.names=F,quote = FALSE,sep='\t')


anno_gene <-max(allRes[,4])
if (anno_gene>2){
	png(paste0(outputpath, "/topGO_BP_",mark, opt$mark, ".png"), width=3000, height=3000, res=300)
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()

	pdf(paste0(outputpath, "/topGO_BP_",mark, opt$mark, ".pdf"))
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()
	print(paste0(outputpath, "/topGO_BP_",mark, opt$mark, ".pdf(png) is OK"));
}

GOdata <- new("topGOdata", ontology = "CC", allGenes = geneList,annot = annFUN.gene2GO, gene2GO = geneID2GO)
resultFisher <- runTest(GOdata, algorithm = "classic", statistic = "fisher")
allRes <- GenTable(GOdata,classicFisher = resultFisher, orderBy = "classicFisher", topNodes = 20)
write.table(allRes,file=paste0(outputpath,"/","CC.",mark,".topGO_RESULT.xls"),row.names=F,quote = FALSE,sep='\t')


anno_gene <-max(allRes[,4])
if (anno_gene>2){
	png(paste0(outputpath, "/topGO_CC_",mark, opt$mark, ".png"), width=3000, height=3000, res=300)
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()

	pdf(paste0(outputpath, "/topGO_CC_",mark, opt$mark, ".pdf"))
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = 'all',.NO.CHAR=0)
	dev.off()
	print(paste0(outputpath,"/topGO_CC_",mark, opt$mark, ".pdf(png) is OK"));
}
