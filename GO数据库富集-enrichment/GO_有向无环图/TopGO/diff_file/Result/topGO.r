...
if (anno_gene>2){
	...
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = "pval") #"all","counts","pval","np"
	dev.off()

	pdf(paste0(outputpath, "/topGO_MF", opt$mark, ".pdf"))
	showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10, useInfo = "pval")
	dev.off()
...
#筛选并且出表格
allRes <- GenTable(GOdata,classicFisher = resultFisher, orderBy = "classicFisher", topNodes = 20)
write.table(allRes,file=paste0(outputpath,"/","BP.topGO_RESULT.xls"),row.names=F,quote = FALSE,sep='\t')
...
resultFisher <- runTest(GOdata, algorithm = "classic", statistic = "fisher")
allRes <- GenTable(GOdata,classicFisher = resultFisher, orderBy = "classicFisher", topNodes = 20)
write.table(allRes,file=paste0(outputpath,"/","CC.topGO_RESULT.xls"),row.names=F,quote = FALSE,sep='\t')
