...
option_list = list(
	...
	make_option(c("-r", "--rot"),type="character", default=90, help="rot_angle, eg: 45,60,90", metavar="character"),
...
df1<-d_l[which(d_l[,1]=="biological process"),]
df1_s<-df1[order(df1[,4],decreasing=T),]
print ("=====================================================")
df2<-d_l[which(d_l[,1]=="cellular component"),]
df2_s<-df2[order(df2[,4],decreasing=T),]
print ("=====================================================")
df3<-d_l[which(d[,1]=="molecular function"),]
df3_s<-df3[order(df3[,4],decreasing=T),]

df_l<-rbind(df1_s,df2_s,df3_s)
dd<-df_l[,c(2,3,4)]
...
Barplot_rot <- function(rot_angle,labels_vec){
	par(mar=c(30,10,8,8),mgp=c(4.5,1,0))
	bp<-barplot(s,width=0.8,space=c(0,0.2),beside=TRUE,border=NA,ylim=c(0,3),las=3,col=colors, args.legend=list(bty="n",cex=2),horiz = FALSE,legend.text=rownames(s),main=paste0("Gene Ontology Classification","(",opt$group,")"),ylab="percentage of genes",axes=F,cex.names=1.5,cex.axis=2,cex.main=3,cex.lab=2,xaxt="n")
	text(bp,y=0, labels = labels_vec, srt = rot_angle, adj = c(1,1.1), xpd = TRUE, cex=1.5) 
#bp<-barplot(s,beside=TRUE,border=NA,ylim=c(0,3),las=3,col=colors, args.legend=list(bty="n",cex=2),horiz = FALSE,legend.text=rownames(s),main=paste0("Gene Ontology Classification","(",opt$group,")"),ylab="percentage of genes",axes=F,cex.names=1.5,cex.axis=2,cex.main=3,cex.lab=2)
	...
	lines(c(1.1,1.1),c(-2.4,-2.58),xpd=T,col="#4DAF4A",lwd=2.5)
	lines(c(1.1,39.7),c(-2.58,-2.58),xpd=T,col="#4DAF4A",lwd=2.5)
	lines(c(39.7,39.7),c(-2.58,-2.4),xpd=T,col="#4DAF4A",lwd=2.5)
	text(29,-2.72,"Biological Process",pos=2,xpd=T,cex=2.4,col="#4DAF4A")
	
	lines(c(41.5,41.5),c(-2.4,-2.58),xpd=T,col="#377EB8",lwd=2.5)
	lines(c(41.5,75.1),c(-2.58,-2.58),xpd=T,col="#377EB8",lwd=2.5)
	lines(c(75.1,75.1),c(-2.58,-2.4),xpd=T,col="#377EB8",lwd=2.5)
	text(68,-2.72,"Cellular Component",pos=2,xpd=T,cex=2.4,col="#377EB8")

	lines(c(76.9,76.9),c(-2.4,-2.58),xpd=T,col="#E41A1C",lwd=2.5)
	lines(c(76.9,112),c(-2.58,-2.58),xpd=T,col="#E41A1C",lwd=2.5)
	lines(c(112,112),c(-2.58,-2.4),xpd=T,col="#E41A1C",lwd=2.5)
...
Barplot_rot(opt$rot,term_list_label2)
dev.off()
