...
print_usage <- function(spec=NULL){
  ...
  cat("
      ...
      --mark          -p  character   mark is shown in the picture title
...
if(is.null(opt$mark)){
	opt$mark="Gene Ontology Classification"
}else{
	opt$mark=paste0(opt$mark, ": ", "Gene Ontology Classification")
}
method<-as.vector(opt$method)
rot<-as.numeric(opt$rotation)
txt.size<-as.numeric(opt$txt.size)
fileName<-as.vector(opt$fileName)
#
ppi=500
...
if(is.na(opt[["method"]])){
  ...
  png(paste(c(outpath,"/",fileName,".png"),collapse =  ""),width =width,height =height,res = ppi,units = "px")
  ...
  text(x=max(le.x),y=3+0.1,labels = sam.lab[1],cex=txt.size,pos=4,xpd=T,font = 2,col="blue")
  ...
  pdf(paste(c(outpath,"/",fileName,".pdf"),collapse =  ""),width =width/ppi,height =height/ppi)
  par(cex=1,cex.lab =txt.size,cex.axis = txt.size,mar = c(txt.size*text.len*0.09*sin(rot*pi/180)/par("csi")+0.5,3,1.5,3),mgp=c(1.6,0.5,0))
  bar=barplot(percent,col=col[indat[,"class"]],border =NA,axes=F,ylim=c(0.1,100),names.arg =NA, ylab="Percentage of genes",space =0.4,)
  title(main=opt$mark,font.main=2,cex.main=0.6)
  mtext(side = 4,line =1.6,text = "Number of genes" ,cex=txt.size,font=2)
...
}else if(opt[["method"]]=="log")
{
  ...
  percent[percent<0]<-0
  png(paste(c(outpath,"/",fileName,".png"),collapse =  ""),width =2000,height=1200,res =300)
  par(cex=1,cex.lab =txt.size,cex.axis = txt.size,mar = c(txt.size*text.len*0.09*sin(rot*pi/180)/par("csi")+0.5,3,1.5,3),mgp=c(1.6,0.5,0))
  ...
  box(bty="n")
  axis(side = 1,at = bar-(bar[2]-bar[1])*0.5,labels =F,tck=-0.02)
  axis(side = 2,at=c(0,1,2,3),lab=c(0.1,1,10,100),cex=txt.size,tck=-0.02)
  text(x=bar, y=-0.02,labels=indat[,2], srt=rot, adj=1, xpd=TRUE,tcl=-0.02,cex=txt.size)
  yl<-c(0.1,1,10,100)*genes.total/100
  yl[yl<1]<-1
  yl<-round(yl)
  axis(side = 4,at=c(0,1,2,3),lab=yl,cex=txt.size,tck=-0.02)
  ...
  pdf(paste(c(outpath,"/",fileName,".pdf"),collapse =  ""),width =10,height =6)
  par(cex=1,cex.lab =txt.size,cex.axis = txt.size,mar = c(txt.size*text.len*0.09*sin(rot*pi/180)/par("csi")+0.5,3,1.5,3),mgp=c(1.6,0.5,0))
  bar=barplot(percent,col=col[indat[,"class"]],border =NA,axes=F,ylim=c(0,3),names.arg =NA, ylab="Percentage of genes", space =0.4,)
  title(main=opt$mark,font.main=2,cex.main=0.6)
  mtext(side = 4,line =1.6,text = "Number of genes" ,cex=txt.size,font=2)
