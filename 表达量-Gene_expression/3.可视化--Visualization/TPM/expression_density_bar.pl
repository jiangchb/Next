...
Options:
	...
	-t     <str>      fpkm or rpm or tpm   [default: fpkm]    [Optional]
...
while(<IN>){
	...
	for (my $i=1; $i<=$#l; $i++){
		...
		if($express_type eq "tpm"){
			if($express < 0.5){
				$express_region{$head[$i]}{"TPM 0-0.5"}++;
			}elsif($express >= 0.5 && $express < 1){
				$express_region{$head[$i]}{"TPM 0.5-1"}++;
			}elsif($express >= 1 && $express < 10){
				$express_region{$head[$i]}{"TPM 1-10"}++;
			}else{
				$express_region{$head[$i]}{"TPM >=10"}++;
			}
		}
	}
}
...
if($express_type eq "tpm"){
	@region=("TPM 0-0.5", "TPM 0.5-1", "TPM 1-10", "TPM >=10");
}
...
if(defined $group_file){
	...
	system("$env && Rscript $Bin/expression_region.r -i $outdir/$express_type\_region.xls -u $express_type -d $group_file -w 8 -t 6 && rm $outdir/$express_type\_region.xls");
}else{
	...
	system("$env && Rscript $Bin/expression_region.r -i $outdir/$express_type\_region.xls -u $express_type -w 8 -t 6 && rm $outdir/$express_type\_region.xls");
}
chdir $pwd;
