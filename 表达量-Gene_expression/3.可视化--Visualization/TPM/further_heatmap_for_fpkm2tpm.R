...
option_list = list(
    make_option(c("-i", "--input"), type = "character", default = NULL,
        help = "diff fpkm matrix file name(force). "),
    make_option(c("-e", "--expression"), type = "character", default = NULL,
        help = "expression TPM matrix file name(force). "),
    ...
    make_option(c("-y", "--colors"), type = "character", default = NULL,
    ...
    make_option(c("-c", "--cutrows"), type = "double", default = "1",
        help = "cutting the trees into several sections,[ default: 1] . ")
);
...
if (! is.null(opt$input)) {
    diff <- read.table(normalizePath(opt$input), header=T, sep="\t", row.names=1, check.names=F, quote="")
#    diff <- subset(diff,select="^Expression_")
    tmp = colnames(diff)
#    print (tmp)
#    str_detect(tmp, "^Expression_")
#    str_extract(tmp,"^Expression_")
#    str_match_all(tmp,"^Expression_")
}else {
    print_help(opt_parser)
    stop("diff FPKM matrix file is not supplied, heatmap for replicate groups  will be skipped\n", call. = FALSE)
}

if (! is.null(opt$expression)) {
    expression <- read.table(normalizePath(opt$expression), header=T, sep="\t", row.names=1, check.names=F, quote="")
    print (tmp)
    expression = as.data.frame(subset(expression,select = tmp))
    tmp_row = row.names(diff)
    expression = as.data.frame(t(subset(t(expression),select = tmp_row)))
}else {
    print_help(opt_parser)
...
############处理文件名############
groupname1 <- gsub("\\.(txt|xls)$", "", gsub("(-diff-pval-0.05-FC-1.5.gene.xls|-diff-pval-0.05-FC-2.gene.xls)$", "", basename(opt$input)))
groupname <-  gsub("^fpkm_", "", groupname1)

output_dir = paste0(opt$outputdir,"/",groupname,"/")
...
if ( !is.null(opt$colors) ){
    ...
    }else if ( opt$colors == "viridis" ){
        palette <- colorRampPalette(viridis(3))(n=256)
    }
}else {
    palette <- colorRampPalette(c("navy","white","firebrick3"))(n=256)#default
}
...
if (length(samples) < 2) {
...
}else if (length(samples) == 2) {#Zscore
    if (opt$rowcluster == T ) {
        #n[n>2]=2 #限定上限，使表达量大于2的等于2
        #n[n< -2]= -2 #限定下限，使表达量小于-2的等于-2
        p = pheatmap(as.data.frame(scale(log2(expression+0.0001))),#scale="row", #as.data.frame(scale(log2(expression+0.0001))),log2(expression+0.0001)
        main = title,
        treeheight_row=30, lwd=1,
        color = palette, show_rownames = show_rownames, fontsize = fontsize, border = border,
        cluster_rows = T , cluster_cols = F , angle_col = angle, border_color = "white",cutree_rows=opt$cutrows)
        data = pheatmap(as.data.frame(scale(log2(expression+0.0001))))
        ...
        cluster_rows = F , cluster_cols = F , angle_col = angle,border_color = "white" )
    }
}else {
    p = pheatmap(log2(expression+0.0001),
    ...
    color = palette, show_rownames = show_rownames, fontsize=fontsize, border=border,
    cluster_rows = opt$rowcluster , cluster_cols = opt$colcluster , angle_col = angle,border_color = "white",cutree_rows=opt$cutrows)
}
...
##########################切割树之后得到表格########################################
## 对基因进行分类
row_cluster <- cutree(p$tree_row,k=opt$cutrows)
...
print ("#######################输出分类结果########################")
data_final<-subset(newOrder,select=c("id","Cluster"))
## 输出结果
#rite.table(data_final,file=paste0(output_dir ,"/", picname,".listout.xls"),quote = F,row.names = F,col.names = T,sep="\t")
