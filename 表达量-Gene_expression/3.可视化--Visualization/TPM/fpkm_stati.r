...
if ( is.null(opt$outputdir) ){
    output_dir = "1.3.gene_expression"
}else{
...
if (is.null(opt$group)) {
    ...
    ggsave(paste0(output_dir,"/",opt$type,"_boxplot.pdf"), height = 8, width = 10+0.2 * length(colnames(data)),plot=p)
    ggsave(paste0(output_dir,"/",opt$type,"_boxplot.png"), type="cairo-png",height = 8, width = 10+0.2 * length(colnames(data)),plot=p)
}else
{   group <- read.table(opt$group, header = T, sep = "\t", check.names = F, quote = "")
    ...
    write.table(MyData, paste0(output_dir,"/","MyData",".xls"), row.names = F, quote = F, sep = "\t")
    ...
    ggsave(paste0(output_dir,"/",opt$type,"_boxplot.pdf"), height = 10, width = 10+0.2 * length(colnames(data)),plot=p)
    ggsave(paste0(output_dir,"/",opt$type,"_boxplot.png"), type="cairo-png",height = 10, width = 10+0.2 * length(colnames(data)),plot=p)
}
...
write.table(expression.stat, paste0(output_dir,"/","report.", opt$type, ".xls"), row.names = F, quote = F, sep = "\t")
