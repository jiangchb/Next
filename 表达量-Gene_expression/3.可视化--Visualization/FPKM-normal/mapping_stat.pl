...
print OUT "Sample\t".join("\t",@keys)."\n";
for my $k1 (@sample_arr){
	print OUT "$k1";
	for my $k2 (@keys){
		print OUT "\t$map_stat{$k2}{$k1}";
	}
	print OUT "\n";
}
close OUT;

#system ( awk -F'\t' -v OFS='\t' '{for(i=0;++i<=NF;)a[i]=a[i]?a[i] FS $i:$i}END{for(i=0;i++<NF;)print a[i]}' OUT >test.xls )
