...
if ( !is.null(opt$genelist) ) {
    ...
    if (length(rownames(labels)) <= 15) {
        ...
        theme(legend.position="bottom") +theme(axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15)) +
        theme(plot.title = element_text(hjust = 0.5, vjust = 0.5)) + 
        guides(shape=guide_legend(override.aes=list(size=6))) +
        scale_colour_manual(values = c("Up"=c("red"), "Down"=c("blue"),"No change"=c("grey")),na.translate=FALSE) + 
        geom_label_repel(aes(label = label), size = 5, vjust=-0.5, force = 1,color = "black") + 
