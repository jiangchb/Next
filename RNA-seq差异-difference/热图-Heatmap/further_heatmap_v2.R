...
option_list = list(
    ...
    make_option(c("-cut", "--cutrows"), type = "double", default = "1",
...
print ("#######################输出分类结果########################")
data_final<-subset(newOrder,select=c("gene_id","Cluster"))
## 输出结果
write.table(data_final,file=paste0(output_dir ,"/", picname,".listout.xls"),quote = F,row.names = F,col.names = T,sep="\t")
