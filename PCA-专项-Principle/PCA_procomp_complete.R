...
path1 <- "gene_counts.xls"
path2 <- "sample_group.txt"
...
pca <- prcomp(t(counts_new), center=T, scale.=F)
...
pca1 = ggplot(pcaData, aes(PC1, PC2, color = Group)) +
  ...
  scale_x_continuous(limits = c(min * 1.7, max * 1.3)) +
  geom_vline(xintercept = 0, linetype = 4, color = "grey") +
  geom_hline(yintercept = 0, linetype = 4, color = "grey") +
#  stat_ellipse(aes(fill = Group, group = Group),geom = "polygon", level = 0.95, alpha = 0.1) +#
  ggforce::geom_mark_ellipse(aes(fill = Group,           #ggforce 可以强制绘图，小于3个样本的也行
                                 color = Group)) +
  scale_alpha_discrete(range = c(0.5,1))+#fill = Group,
  coord_equal()
ggsave("PCA_1_test.pdf", height = 8, width = 8,pca1)
ggsave("PCA_1_test.png",type="cairo-png",height=8,width=8,pca1)


pca2 = pca1 + geom_text_repel(aes(x = PC1, y = PC2, label = rownames(pcaData)), size = 3) 

ggsave("PCA_2_test.pdf", height = 8, width = 8,pca2)
ggsave("PCA_2_test.png",type="cairo-png",height=8,width=8,pca2)


#pca3D
labels <- rainbow(1 : length(seq_along(levels(pcaData$group))))
