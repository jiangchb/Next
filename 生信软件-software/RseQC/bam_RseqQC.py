...
def read_distribution(bed, bam, output, prefix):
    ...
    if files.exists() and len(files.read_text()) > 0:
        ...
        command2 = "/home/lipeng/lipeng_script/pipline/Prok_Automata/1.Trimming/read_distribution.py -r {}  -i {} > {}/{}.read_distribution.txt".format(bed, bam, output, prefix)
        print(command2)
        subprocess.run(command2, shell=True, check=True)
        odata = pd.read_table("./" + prefix + ".read_distribution.txt", header=None, sep='\s+', skiprows=5, nrows=9,
        ...
        df.plot(kind='bar',
                color=['#9BCD9B', '#9BCD9B', '#9BCD9B', '#C1FFC1', 'lightskyblue', 'lightskyblue', 'lightskyblue',
                       'lightcoral', 'lightcoral'], legend=None, figsize=(8, 8), fontsize=8, width=0.7)
        plt.ylabel('Tag_count', fontsize=9)
        plt.xlabel('Group', fontsize=9)
